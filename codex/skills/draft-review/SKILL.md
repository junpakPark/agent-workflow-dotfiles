...
3a. Treat long-running Claude review silence as normal. Claude `--output-format json` may produce no stdout until the process exits, so silence is expected and is not a stall signal. After spawning Claude, poll process liveness every 30 seconds. Before 10 minutes elapsed, wait without user-facing progress reports unless the process exits. At 10 minutes elapsed and every 1 minute after that, report that the checkpoint is still running with child id, checkpoint name, PID when available, and elapsed time. At 15 minutes elapsed, ask the user whether to terminate; continue waiting by default if the user does not explicitly request termination. Do not kill the process for inactivity. Do not launch a duplicate Claude review while a matching child/checkpoint process is still live. If the user explicitly terminates or the host interrupts the process before a wrapper is emitted, preserve only a debug artifact and require explicit user approval before any same-input retry because the prior call may already have consumed tokens.

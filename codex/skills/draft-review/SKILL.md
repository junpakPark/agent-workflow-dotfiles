...
2. Invoke Claude `draft-intent-worker` with a prompt that names the installed worker path (`${HOME}/.claude/skills/draft-intent-worker/SKILL.md`) and supplies the parent/child paths and prior findings. If the Claude CLI is unavailable or returns non-JSON, stop and report the blocker.
